...
type Props = {
  name: string;
  number_of_activities: string | number;
  image: string;
  rating: string | number;
};

function SingleFeatured({ name, number_of_activities, image, rating }: Props) {
  return (
    <div className="relative rounded-lg overflow-hidden group sm:cursor-pointer">
      ...
      />
      <span className="absolute top-2 left-2 px-4 py-[1px] bg-white text-[#ff543d] rounded-full">
        {rating}
      </span>
